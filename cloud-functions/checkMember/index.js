...
const requestAsync = async (options) => {
  return new Promise((resolve, reject) => {
    request(options, (err, resp, body) => {
      if (err) {
        reject(new Error(`Request failed with error: ${err.message}`));
      } else if (resp.statusCode !== 200) {
        reject(
          new Error(`HTTP status ${resp.statusCode}: ${JSON.stringify(body)}`)
        );
      } else {
        resolve(body);
      }
...
app.post("/auth", async (req, res) => {
  try {
    ...
    const [licenses, userData] = await Promise.all([
      getWhopData(
        "memberships?page=1&per=10&user_id=",
        userId + "&hide_metadata=false"
      ),
    ...
    const licenseObjects = await Promise.all(
      licenses.data
        .filter(({ status }) =>
          ["active", "trialing", "past_due", "completed"].includes(status)
        )
        .map(async (license) => {
          ...
          return {
            ...
            price:
              planData[
                planData.card_payments ? "renewal_price" : "initial_price"
              ],
...
app.post("/stripe", async (req, res) => {
  ...
  if (!userId) {
    return res
      .status(400)
      .json({ success: false, error: "User name is missing" });
  }
  ...
  if (!userDoc.exists) {
    return res
      .status(404)
      .json({ success: false, error: "User not found for stripe" });
  }
  ...
  if (!mem_id) {
    return res
      .status(400)
      .json({ success: false, error: "Missing mem_id parameter" });
  }
  ...
  if (!return_url) {
    return res
      .status(400)
      .json({ success: false, error: "Missing return_url parameter" });
  }
  ...
  try {
    ...
    if (!whopData.stripe_customer_id) {
      return res
        ...
        .json({
          success: false,
          error: "Whop membership data does not include stripe_customer_id",
        });
    }
...
app.post("/cancel", async (req, res) => {
  ...
  if (!userDoc.exists) {
    return res
      .status(404)
      .json({ success: false, error: "User not found for cancel" });
  }
  ...
  try {
    ...
    res
      ...
      .json({
        success: true,
        message: "Whop membership cancelled successfully",
        response,
      });
  } catch (error) {
    console.error(error);
    res
      .status(500)
      .json({ success: false, message: "Failed to cancel Whop membership." });
  }
});
