    ...
    try {
      ...
      userRef.update(updateFields)
        .then(() => {
          ...
          res.status(200).json({ success: true, message: `Exchange information updated successfully - ${traderId}!` });
        })
        .catch((error) => {
...
app.get('/account', async (req, res) => {
  ...
  try {
    ...
    const productsCollectionSnapshot = await db.collection('traders').doc(traderId).collection('products').get();
...
app.delete('/deleteExchange', async (req, res) => { // Removed traderId from the route
  const traderId = req.get('traderId');
