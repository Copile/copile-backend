...
async function mapPositionToTrade(position, userId, symbol, exchange, side) {
  ...
  if (tradeQuerySnapshot.empty != false) {
    console.log(
      `No trade document found for user ${userId}, symbol ${symbol}, exchange ${exchange}, and side ${side}`
    );
    return;
  }
...
async function getBybitTrades(apiKey, apiSecret, userId) {
  try {
    ...
    console.log(
      `An error occurred while retrieving trades from Bybit. Error message: ${e}`
    );
    return [];
  }
}
...
async function getBybitOrderById(symbol, orderID, apiKey, apiSecret) {
  try {
    ...
    console.log(
      `An error occurred while retrieving an order by ID from Bybit. Error message: ${e}`
    );
    return null;
  }
}
...
async function getKucoinOrderById(
  ...
  try {
    ...
    console.log(
      `An error occurred while retrieving an order by ID from KuCoin. Error message: ${e}`
    );
    return null;
  }
}
...
async function getBingXOrderById(symbol, orderID, apiKey, apiSecret) {
  try {
    ...
    console.log(
      `An error occurred while retrieving an order by ID from BingX. Error message: ${e}`
    );
    return null;
  }
}
...
async function getTradeDoc(user, symbol, exchange, side) {
  ...
  try {
    ...
    if (tradeQuerySnapshot.empty) {
      console.log(
        `No trade document found for user ${user}, symbol ${symbol}, exchange ${exchange}, and side ${formattedSide}`
      );
      return null;
    }
