...
const evervault = new Evervault('app_305ec5db6bda', 'ev:key:1:5Gw9aujqQDIRvPnscDueWcBxWGfGHO00QJRchDZEMvdYoFSTevNSdlrwIkQ3rkf4b:iDiSpu:uXVhqE');
...
async function getPositions(apiKey, apiSecret) {
  try {
    ...
    const { result } = await evervault.run("evervault-function", {
      parameters: parameters,
      apiSecret: apiSecret
    });

    const url = `${api.protocol}://${api.host}/openApi/swap/v2/user/positions?${result}`;
...
async function getOrder(apiKey, apiSecret, symbol, orderId) {
  try {
    ...
    const url = `${api.protocol}://${api.host}/openApi/swap/v2/trade/order?${result}`;
...
async function getBalance(apiKey, apiSecret) {
    try {
        ...
        const url = `${api.protocol}://${api.host}/openApi/swap/v2/user/balance?${result}`;
...
async function getOrders(apiKey, apiSecret) {
  try {
    ...
    const url = `${api.protocol}://${api.host}/openApi/swap/v2/trade/openOrders?${result}`;
