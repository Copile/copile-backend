...
app.all('/trades/:exchange', async (req, res) => {
  ...
  try {
    ...
    const userRef = db.collection('traders').doc(trader);
    ...
    const apiSecret = keys.api_secret || null;
    ...
    switch (exchange) {
      case 'kucoin':
        trades = await getKucoinTrades(apiKey, apiSecret, apiPassphrase, trader);
        orders = await getKucoinOrders(apiKey, apiSecret, apiPassphrase, trader);
        break;
      case 'bingx':
        trades = await getBingXTrades(apiKey, apiSecret, trader);
        orders = await getBingXOrders(apiKey, apiSecret, trader);
        break;
      default:
    ...
    return res.json({ success: true, exchange: exchange, trades: paginatedTrades, orders: orders, executionTime });
...
async function getTradeProfitLossDetails(trader, tradeId, exchange, symbol, apiKey, apiSecret, apiPassphrase = null) {
  try {
    ...
    takeProfitData = await checkTakeProfitStatus(exchange, symbol, takeProfitData, apiKey, apiSecret, apiPassphrase);
    stopLossData = await checkStopLossStatus(exchange, symbol, stopLossData, apiKey, apiSecret, apiPassphrase);
...
async function getKucoinOrders(apiKey, apiSecret, apiPassphrase, trader) {
    try {
        ...
        const kucoinMatchingParams = await Promise.all(filteredOrders.map(async order => {
          const tradeId = await getTradeDoc(trader, order.symbol, "kucoin", order.side);
          return {
            trade_id: tradeId,
            symbol: order.symbol,
            side: order.side,
...
async function getBingXOrders(apiKey, apiSecret, trader) {
  try {
    ...
    const bingxMatchingParams = await Promise.all(orders.map(async order => {
      const tradeId = await getTradeDoc(trader, order.symbol, "bingx", order.side);
      return {
        trade_id: tradeId,
        symbol: order.symbol,
        side: order.side,
