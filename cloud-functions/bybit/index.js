...
app.all('/trades', async (req, res) => {
  ...
  if (!entity) {
    return res.status(404).json({ success: false, error: `${entityCollection.slice(0, -1)} not found` });
  }
...
app.all('/balance', async (req, res) => {
    ...
    try {
        ...
        if (!exchangesData || !('bybit' in exchangesData)) {
            return res.status(404).json({ success: false, error: 'No exchange found or unsupported exchange' });
        }
...
app.get('/symbolData', async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({ success: false, error: 'Failed to fetch data from the external API' });
  }
});
...
async function mapPositionToTrade(position, entityId, symbol, exchange, side, entityCollection) {
  ...
  if (tradeQuerySnapshot.empty != false) {
    console.log(`No trade document found for entity ${entityId}, symbol ${symbol}, exchange ${exchange}, and side ${side}`);
    return null;
  }
  ...
  try {
    ...
    const trades = positionData.result.list
      ...
      .map(async position => {
        ...
        return await mapPositionToTrade(position, entityId, position.symbol, "bybit", position.side, entityCollection);
      });
...
async function getTradeProfitLossDetails(entityCollection, entityId, tradeId, exchange, symbol, apiKey, apiSecret) {
  try {
    const tradeDocRef = db.collection(entityCollection).doc(entityId).collection('trades').doc(tradeId);
    ...
    takeProfitData = await checkTakeProfitStatus(exchange, symbol, takeProfitData, apiKey, apiSecret);
    stopLossData = await checkStopLossStatus(exchange, symbol, stopLossData, apiKey, apiSecret);
...
async function checkTakeProfitStatus(exchange, symbol, takeProfitData, apiKey, apiSecret) {
  for (const tp of takeProfitData) {
    if (tp.executed === "0") {
      tp.tp_status = "Queued";
    } else if (tp.executed === "1") {
      console.log(tp);
      let status = await getOrderStatus(exchange, symbol, tp.orderID, apiKey, apiSecret);
...
async function checkStopLossStatus(exchange, symbol, stopLossData, apiKey, apiSecret) {
  for (const sl of stopLossData) {
    if (sl.executed === "0") {
      sl.sl_status = "Queued";
    } else if (sl.executed === "1") {
      let status = await getOrderStatus(exchange, symbol, sl.orderID, apiKey, apiSecret);
...
async function getOrderStatus(exchange, symbol, orderID, apiKey, apiSecret) {
  const bybit_order = await getBybitOrderById(symbol, orderID, apiKey, apiSecret);
...
async function getBybitOrders(apiKey, apiSecret, entityCollection, entityId) {
  try {
    ...
    const bybitMatchingParams = await Promise.all(filteredOrders.map(async order => {
      const tradeDoc = await getTradeDoc(entityCollection, entityId, order.symbol, "bybit", order.side);
      const tradeData = tradeDoc.data();
      return {
