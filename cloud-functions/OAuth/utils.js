...
async function generateChatToken(user) {
  try {
    ...
    if (
      user_data.exists &&
      user_data.data().telegram &&
      user_data.data().telegram.token != "x"
    ) {
...
async function saveUserDataToFirestore(user, user_data, social) {
  try {
    ...
    if (
      ...
      user_snapshot.data()[social] &&
      user_snapshot.data()[social].id != "x"
    ) {
      if (social == "telegram") {
        await sendTelegramMessage(
          user_snapshot.data().telegram.id,
    ...
    throw new ApiError(
      500,
      `Error saving ${social} user data to firestore.`,
      social
    );
  }
}
