...
app.post('/new_membership', async (req, res) => {
  ...
  try {
    ...
    const membersCollection = db.collection('traders').doc(planDoc).collection('products').doc(planId).collection('members');
    ...
    return res.status(200).json({ message: 'Membership created successfully.' });
  } catch (error) {
    console.log(error);
...
app.post('/delete_membership', async (req, res) => {
  ...
  try {
    ...
    if (!membershipDocSnapshot.exists) {
      return res.status(404).json({ error: `Membership document with id ${userId} not found.` });
    }
...
app.post('/payment', async (req, res) => {
  ...
  try {
    ...
    const salesCollection = db.collection('traders').doc(planDoc).collection('products').doc(planId).collection('sales');
