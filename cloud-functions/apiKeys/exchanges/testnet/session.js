...
const { getBybitAPIPerms } = require("../bybit/apiPerms");
...
class TestnetSession extends ExchangeSession {
  ...
  async getAPIPerms() {
    try {
      return await getBybitAPIPerms(this.apiKey, this.apiSecret, true);
    } catch (e) {
      if (e instanceof CustomError) {
        throw e;
      }
      throw new CustomError({
        message: `Failed to fetch testnet APIPerms: ${e.message}`,
        status: 500,
        source: "getAPIPerms",
      });
    }
