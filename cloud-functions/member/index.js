...
app.post('/updateMargin', async (req, res) => {
  ...
  const { license_key, margin, percentage, option, preferred_exchange } = req.body;
  ...
  try {
    ...
    if (planQuerySnapshot.empty) {
      return res.status(404).json({success:false, error: 'Plan not found for this user and license key' });
    }

    // Update plan document with new margin, percentage, option, preferred_exchange values
    ...
    await planDocRef.update(updateFields);
