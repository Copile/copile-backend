...
import os
import datetime
from .utils.firestore import trader_check
...
@app.post('/newTrade')
async def new_trade(data: dict, traderId: Optional[str] = Header(None), status_code=200):
    try:
        ...
        await add_task_to_queue("newTrade", trade_data)
...
@app.post('/submitTP')
async def submit_tp(data: dict, traderId: Optional[str] = Header(None), status_code=200):
    try:
        exists = await trader_check(traderId)
        ...
        # Add the trade to the processing queue
        await add_task_to_queue("submitTP", trade_data)

        return JSONResponse(status_code=200, content={"success": True, "message": 'Take-profit submitted successfully.'})
    except Exception as err:
        # Log the error and return an error response
        print(err)
        raise HTTPException(status_code=500, detail=str(err))
...
async def submit_sl(data: dict, traderId: Optional[str] = Header(None), status_code=200):
    try:
        ...
        # Add the trade to the processing queue
        await add_task_to_queue("submitSL", trade_data)
...
async def cancel_all_orders(data: dict, traderId: Optional[str] = Header(None), status_code=200):
    try:
        ...
        # Add the trade to the processing queue
        await add_task_to_queue("cancelAllOrders", trade_data)
...
async def cancel_all_tps(data: dict, traderId: Optional[str] = Header(None), status_code=200):
    try:
        ...
        # Add the trade to the processing queue
        await add_task_to_queue("cancelAllTPs", trade_data)
...
async def bulk_order(data: dict, traderId: Optional[str] = Header(None), status_code=200):
    try:
        ...
        # Add the trade to the processing queue
        await add_task_to_queue("bulkOrder", trade_data)
...
async def bulk_tp(data: dict, traderId: Optional[str] = Header(None), status_code=200):
    try:
        ...
        # Add the trade to the processing queue
        await add_task_to_queue("bulkTP", trade_data)
...
async def replace_sl(data: dict, traderId: Optional[str] = Header(None), status_code=200):
    try:
        ...
        # Add the trade to the processing queue
        await add_task_to_queue("replaceSL", trade_data)
...
async def partial_close(data: dict, traderId: Optional[str] = Header(None), status_code=200):
    try:
        ...
        # Add the trade to the processing queue
        await add_task_to_queue("partialClose", trade_data)
