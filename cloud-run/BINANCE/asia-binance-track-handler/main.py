...
async def bulk_tp(data: dict):
    ...
    if new_take_profits != []:
        ...
        exchange = trade_info["exchange"]
        await binance.check.check_trades(account_id, trade_id, new_take_profits, trade_info, "/bulk_tp", user_type)
...
async def check_trade(account_id, trade_id, document_id, payload, exchange, trade_info, endpoint, user_type):
    try:
        await binance.check.check_trade(account_id, trade_id, document_id, payload, trade_info, endpoint, user_type)
