...
async def replace_sl(data: dict):
    ...
    try:
        ...
        # Cancel the order
        await binance.cancel.send_cancel(account_id, trade_id, order_id, "tp", trade_info, keys)

        # Resend the tp with the updated payload
        await binance.profit.send_profit(account_id, trade_id, payload['tp_id'], payload['tp_number'], payload['tp_value'], payload['tp_percentage'], float(tp_quantity), trade_info, precision, keys)
