...
from .settings import change_partial_mode
import asyncio
...
async def check_trade(account_id, trade_id, document_id, payload, trade_info, endpoint, user_type):
    try:
        ...
        position = await get_position(account_id, symbol, trade_info, keys)
        
        if float(position) != 0:
            precision, partial_mode = await asyncio.gather(
                get_precision(account_id, symbol, keys),
                change_partial_mode(symbol, keys)
            )
