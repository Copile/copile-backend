...
async def send_stoploss(account_id, trade_id, sl_document_id, sl_number, sl_value, sl_percentage, sl_amount, position, trade_info, keys):
    try:
        ...
        if str(position['tpslMode']) != "Partial":
            try:
        ...
        sl_amount = round(abs(float(quantity)) * float(sl_percentage), quantity_precision) if sl_amount is None else sl_amount
