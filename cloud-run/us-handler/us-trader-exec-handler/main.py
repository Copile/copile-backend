...
from exchanges import kucoin, bingx
...
async def replace_tp(data: dict):
    ...
    try:
        ...
        current_quantity = EXCHANGES[exchange].order.get_order_quantity(account_id, trade_id, order_id, trade_info, keys)
...
async def bulk_order(data: dict):
    ...
    try:
        ...
        trade_result = await EXCHANGES[exchange].trade.send_trade(account_id, trade_id, margin, side, symbol, leverage, entry, keys)
