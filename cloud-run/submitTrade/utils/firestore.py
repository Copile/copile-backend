...
async def trader_check(trader_id):
    try:
        document_ref = db.collection(COLLECTION_TRADERS).document(trader_id)
        document_snapshot = await document_ref.get()

        return document_snapshot.exists

    except Exception as e:
        logger = Logger(trader_id, None)
        logger.error(e)
        raise e
...
async def store_trade(account_id, order_dict):
    try:
        ...
        logger = Logger(account_id, order_dict['trade_id'])
        logger.error(e)
        raise e
...
async def store_tp(account_id, tp_dict):
    try:
        ...
        logger = Logger(account_id, tp_dict['trade_id'])
        logger.error(e)
        raise e
...
async def store_sl(account_id, sl_dict):
    try:
        ...
        logger = Logger(account_id, sl_dict['trade_id'])
        logger.error(e)
        raise e
...
async def delete_order(account_id, trade_id):
    try:
        ...
        logger = Logger(account_id, trade_id)
        logger.error(e)
        raise e
