...
async def send_tp(data: dict):
    ...
    if document_status.exists:
        ...
        if executed == "0":
            return await check_trade(account_id, trade_id, tp_document_id, payload, "bingx", trade_info,"/send_tp", user_type)
        else:
...
async def bulk_tp(data: dict):
    ...
    if new_take_profits != []:
        ...
        await bingx.check.check_trades(account_id, trade_id, new_take_profits, trade_info, "/bulk_tp", user_type)
...
async def send_sl(data: dict):
    ...
    if document_status.exists:
        executed = document_status.to_dict()["executed"]
        if executed == "0":
            return await check_trade(account_id, trade_id, sl_document_id, payload, "bingx", trade_info, "/send_sl", user_type)
        else:
...
async def check_trade(account_id, trade_id, document_id, payload, exchange, trade_info, endpoint, user_type):
    try:
        await bingx.check.check_trade(account_id, trade_id, document_id, payload, trade_info, endpoint, user_type)
