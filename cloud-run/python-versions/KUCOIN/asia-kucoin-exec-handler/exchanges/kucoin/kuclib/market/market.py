...
class MarketData(KucoinBaseRestApi):
    ...
    async def get_contract_detail(self, symbol):
        ...
        """
        return await self._request('GET', '/api/v1/contracts/{symbol}'.format(symbol=symbol), auth=False)
    ...
    async def get_symbol_list_v2(self, **kwargs):
        """
        https://docs.kucoin.com/#get-symbols-list
        :param kwargs: [Optional] market
        :return:
        ...
        if kwargs:
            params.update(kwargs)
        return await self._request('GET', '/api/v2/symbols', params=params)
