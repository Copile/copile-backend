...
def encoded_string(query, special=False):
    """Encode a dict into a URL query string, handling array parameters."""
    encoded_items = []
    for k, v in query.items():
        if isinstance(v, str) and v.startswith('[') and v.endswith(']'):
            encoded_items.append(f"{k}={v}")  # Directly append without urlencode
        else:
            encoded_item = urlencode({k: v}).replace("%40", "@").replace("%27", "%22")
